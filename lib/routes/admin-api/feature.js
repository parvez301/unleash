...
const webhook = require('../../update-cache-webhook')
...
class FeatureController extends Controller {
    ...
    async createToggle(req, res) {
        ...
        const features = await this.featureToggleStore.getFeatures();
        let splitName = toggleName.split(".")

        let casName = splitName[0];
        let envName = splitName[1];

        let webhookUrl = webhook.mapCasWithEnvWebhookUrls[casName][envName];

        webhook.updateCacheWebhook(features, webhookUrl);
    }
    ...
    async updateToggle(req, res) {
        ...
        let splitName = featureName.split(".")
